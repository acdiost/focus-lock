...
- **System tray** — hides to the tray when the main window is closed; right-click menu shows live timer state and phase controls
...
> The Linux build requires `libwebkit2gtk-4.0-37`, `libgtk-3-0`, and `libayatana-appindicator3-1` (tray support). Ubuntu 22.04+ ships the first two; install the tray library with `sudo apt install libayatana-appindicator3-1`.
...
| [Rust](https://rustup.rs/) stable 1.77+ | Backend compiler |
...
> Tray support is enabled via the `tray` Cargo feature, which is already wired into the npm scripts — no manual flags needed.

### Code signing (optional)

The default configuration ships unsigned. For notarized distribution, update `src-tauri/tauri.conf.json`:
...
│   └── style.css           # Styles
├── src-tauri/
│   ├── src/main.rs         # All backend logic (~900 lines)
...
| `build_tray_menu` | Dynamically builds the tray right-click menu based on current phase; rebuilt on every state push |
| `tray_icon` | 22×22 anti-aliased clock face drawn pixel-by-pixel in Rust; uses macOS template mode to auto-adapt to light/dark themes |
...
- [Tauri v1](https://tauri.app/) — Rust backend + WebView shell
